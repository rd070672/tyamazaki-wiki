# JuKKR（Jülich KKR codes）による電子状態計算：グリーン関数DFTで磁性・不純物・輸送をつなぐ

JuKKRは、KKR（Korringa–Kohn–Rostoker）グリーン関数法に基づくオールエレクトロンDFTコード群である。周期系から不純物埋め込み、散乱・輸送までを、同一の多重散乱形式で一貫して扱える点が特徴である。
