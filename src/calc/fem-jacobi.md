# ヤコビ法（Jacobi反復法）と並列計算：定常反復・緩和・領域分割の要点

ヤコビ法は、巨大かつ疎な連立一次方程式 $A\mathbf{x}=\mathbf{b}$ を、行列の対角成分を用いた反復更新で解く定常反復法である。差分化・有限要素化で現れるポアソン型・拡散型・弾性型の線形方程式を、単純な構造で扱える点が特徴である。
...
## 4. 緩和ヤコビ法（Weighted / Relaxed Jacobi）
ヤコビ法は単純である一方、収束が遅い場合が多い。そこで緩和係数 $\omega$ を導入した緩和ヤコビ法が用いられる。
$$
\mathbf{x}^{(k+1)}
=
(1-\omega)\mathbf{x}^{(k)}
