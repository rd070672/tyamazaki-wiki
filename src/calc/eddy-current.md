...
## 3. 1次元磁壁モデルによる見積もり：$\partial\mathbf{M}/\partial t$ の局在
...
## 9. 近縁概念：スピンモーティブフォース（SMF）との切り分け
...
| 壁速度 | $v$ | m/s 程度 | 大きいほど誘導が強くなる |
| 透磁率 | $\mu$ | 大 | 拡散・表皮に影響し、損失の形を変える |
| 磁区構造 | 統計量 | 材料依存 | 壁密度・ピン止め統計が $P_{\mathrm{ex}}$ と MBN を規定する |
