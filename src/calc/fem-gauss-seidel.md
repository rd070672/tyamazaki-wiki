# ガウス・ザイデル法：連立一次方程式の基本反復解法と緩和（smoother）
...
### 2.2 行列分割（matrix splitting）による表現
係数行列を
$$
A = D + L + U
$$
...
## 7. 多重格子法におけるガウス・ザイデル：高周波誤差の減衰（smoothing）
...
## 8. 並列化の工夫：赤黒（red-black）・ブロック化・マルチカラー
