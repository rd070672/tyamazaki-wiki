# 量子モンテカルロ計算（QMC）：多体量子系を確率過程で解く枠組みと固体系への適用
...
1) 波動関数（実空間）型 QMC  
連続空間の電子を座標 $\mathbf{R}=(\mathbf{r}_1,\dots,\mathbf{r}_N)$ で扱い、試行波動関数 $\Psi_T(\mathbf{R})$ を用いる。代表例は VMC と DMC である。
...
## 6. AFQMC：補助場で相互作用を分解し、スレーター行列式空間で歩く
...
国内向けの導入資料として、大学・研究所の講義ノートやアプリ情報サイトにQMCの概説やリンク集が公開されている。
