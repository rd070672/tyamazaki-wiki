# モンテカルロ法の数値解法：サンプリング設計・収束判定・誤差評価の手順
...
### 3.1 要件：定常分布・エルゴード性・（しばしば）詳細釣り合い
