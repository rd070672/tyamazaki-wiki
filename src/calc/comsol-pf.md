...
- COMSOL Multiphysics では、相変態（固相／液相、析出、スピノーダル分解、マルテンサイト変態など） を
  ...
  を組み合わせることで解析できる。
- 基本的な考え方：
  - 秩序パラメータ（オーダーパラメータ）$\phi$ や濃度 $c$ を連続場として扱う。
  - 自由エネルギー汎関数 $F[\phi,c,T,\varepsilon]$ を定義し、
  - その変分から得られる時間発展方程式（Allen–Cahn, Cahn–Hilliard など）を COMSOL の PDE として実装する。
