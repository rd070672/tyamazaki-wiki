# 虚数時間発展法（Imaginary-Time Evolution）の基礎と計算物性への展開

虚数時間発展法は、時間発展方程式の形式を利用して基底状態（最低エネルギー固有状態）を数値的に抽出する反復法である。固有値問題を「拡散・緩和」問題へ置き換えることで、安定にエネルギー最小状態へ収束させる枠組みを与える。
...
## 2. 基本原理：Wick回転と基底状態への射影

### 2.1 出発点（実時間のSchrödinger方程式）
$\hbar=1$ として、
$$
