# AkaiKKR（KKR-CPA）による不規則系電子状態計算の原理
...
## 3. CPA（Coherent Potential Approximation）：置換不規則の平均化
置換不規則合金 $A_{x}B_{1-x}$ のように、同一サイトに複数元素が確率的に入る系では、スーパーセルを巨大化して乱数配置を平均する手もあるが、計算は重い。

CPAは、実在の不規則系を「有効媒質（coherent medium）」で置き換え、媒質中に各成分原子を埋め込んだときの平均散乱がゼロになるように媒質を自己無撞着に決める近似である（単一サイトCPAが基本である）。
...
相対論効果としては、スカラー相対論やスピン軌道相互作用（SOC）を取り込み、磁気異方性などのSOC起源量に接続する構成が取られることがある（ただし数値条件の詰めが重要である）。
