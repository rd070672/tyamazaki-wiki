# 第一原理計算に基づくフェーズフィールド計算：自由エネルギー汎関数の構築とマルチスケール連成
...
## 3. ワークフロー1：DFT → クラスター展開 → Monte Carlo → $G(c,T)$ → PF
...
## 4. ワークフロー2：DFT → 振動自由エネルギー（フォノン） → 温度依存相安定
...
## 5. ワークフロー3：第一原理＋CALPHAD → PF（熱力学DB連成）
...
## 7. 弾性・固有ひずみ連成：析出、相変態、双晶・マルテンサイト
