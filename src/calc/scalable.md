# 離散化とスケーラブル解法による大規模計算設計：精度・安定性・並列効率の要点
...
### 5.3 領域分割（Domain Decomposition; DD）

計算領域をサブドメインに分割し、局所問題と粗視化問題を組み合わせて解く。有限要素/有限体積の大規模並列では、手法論としても並列実装としても中核になることが多い。
