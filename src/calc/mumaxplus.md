...
- **mumax+** は、mumax3 を拡張して  
  **磁化ダイナミクス（LLG）と弾性場（ひずみ・応力）を連成して扱えるようにしたコード**（系統）であり、
  - 磁歪（magnetostriction）
...
- 基本的な考え方は、
  ...
  - 同じ格子上で **弾性方程式を解き、ひずみ・応力場を更新**
  - 磁歪エネルギーを通じて、**磁化⇔弾性の双方向結合**を入れる  
...
- 磁気弾性エネルギー $F_\text{me}$ に対する変分から、
  ...
  $$
  が得られ、  
  **弾性場（ひずみ・応力）が LLG における有効磁場としてフィードバック**する。
...
- ここで磁歪ひずみ $\varepsilon^\text{mag}_{kl}(\mathbf{m})$ が $\mathbf{m}$ の関数なので、  
  **磁化が変わると弾性場が変わり、弾性場が変わるとまた磁化が変わる**という連成ループが形成される。
