了解である。まず「完全自作で FEM（6面体Q1固定）＋3Dマイクロ磁気（LLG）＋磁気弾性まで」を成立させるには、コードを(1)データ構造、(2)離散化（要素・演算子）、(3)物理（エネルギー→有効磁場）、(4)時間積分、(5)長距離場（反磁界）と弾性場（連立線形系）、(6)入出力・検証、に分割して育てるのが最短である。

以下は「大枠のみ（詳細省略）」として、用意すべき機能・サブルーチンと、main.py から import する前提のフォルダ構造案である（後から各モジュールを順番に埋めれば動く設計にしてある）。
