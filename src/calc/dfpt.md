# 密度汎関数摂動論（DFPT）入門：結晶の線形応答を第一原理で計算する枠組み
...
## 5. 誘電応答・Born有効電荷・圧電・弾性：同じ線形応答の延長
...
## 6. 電子・フォノン相互作用（e-ph）：DFPTで得る相互作用行列要素
