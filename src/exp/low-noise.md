# 低ノイズ・高感度な電圧測定技術：微小信号を「測れる量」に変える原理と実装
...
## 3. 「低ノイズ化」の設計原理：帯域を減らし、差動にし、結合を断つ
...
### 3.4 ガード（guarding）：微小電圧計測で漏れ電流を起こさせない
