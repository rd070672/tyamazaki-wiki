...
## 3.1 共振（resonance）
共振は、ある周波数で系がエネルギーを蓄えやすくなり、ゲインが局所的に増大する現象である。機械構造（治具、プローブ、支持、配線の振動）、電気回路（RLC）、磁気系（磁化ダイナミクス）などが共振源になる。
...
## 3.2 反共振（antiresonance）
反共振は、ある周波数で応答が局所的に落ち込む（ノッチが現れる）現象である。多自由度系の干渉（異なる経路の合成が打ち消し合う）、機械–電気の結合、測定点の位置（節）などで発生する。
