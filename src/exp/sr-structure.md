...
$$
と定義し、振動成分 $\chi(k)$ を抽出することで、近接原子環境を明らかにする。
...
コヒーレント回折イメージング（CDI）では、試料の複素透過関数 $O(\mathbf{r})$ のフーリエ変換強度 $I(\mathbf{q})$ を測り、
$$
...
照明プローブ $P(\mathbf{r})$ を走査（ptychography）し、重なりを持たせた回折データ集合から $O(\mathbf{r})$ を復元する。吸収像だけでなく位相像が得られ、密度分布や界面の微細構造、歪み場の情報へ接続しやすい。また、測定冗長性が位相回復の安定性を高める方向に働く。
